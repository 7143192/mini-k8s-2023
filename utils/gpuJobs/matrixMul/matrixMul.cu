...
// NOTE: a hack here... nx = ny in this cuda program !!!
// res[i][j] = sum(A[i][k] * B[k][j])
__global__
void matrixMul(int* A, int* B, int* res, int nx, int ny) {
    if (nx != ny) {
        return ;
    }
    ...
    for (int i = 0; i < nx; ++i) {
        int tmpA = A[i + ix * nx]; // A[ix][i]
        int tmpB = B[iy + i * nx]; // B[i][iy]
        res[iy + ix * nx] += tmpA * tmpB;
    }
}
...
int main() {
    ...
    // grid (in this cuda program): 64 * 64
    ...
    matrixMul<<<numBlocks, threadsPerBlock>>>(A_dev, B_dev, dev_res, nx, ny);
